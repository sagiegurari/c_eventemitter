...
void _test_cb3(void *event_data, void *context)
{
  assert_string_equal((char *)event_data, "event");
  assert_string_equal((char *)context, "test3");
...
void test_impl()
{
  ...
  id = eventemitter_prepend_once_listener(event_emitter, -1, _test_cb0, "test0");
  assert_num_equal(id, 2);
  assert_num_equal(eventemitter_listeners_count(event_emitter, -1), 2);

  id = eventemitter_once(event_emitter, -1, _test_cb2, "test2");
  assert_num_equal(id, 3);
  assert_num_equal(eventemitter_listeners_count(event_emitter, -1), 3);

  id = eventemitter_add_once_listener(event_emitter, -1, _test_cb3, "test3");
  assert_num_equal(id, 4);
  assert_num_equal(eventemitter_listeners_count(event_emitter, -1), 4);

  _test_global_order_counter = 0;
  int count = eventemitter_emit(event_emitter, -1, "event");
  assert_num_equal(count, 4);
  assert_num_equal(_test_global_counter, 4);
  assert_num_equal(_test_global_order_counter, 4);
  assert_num_equal(eventemitter_listeners_count(event_emitter, -1), 1);

  _test_global_order_counter = 0;
  count                      = eventemitter_emit(event_emitter, -1, "event");
  assert_num_equal(count, 1);
  assert_num_equal(_test_global_counter, 5);
